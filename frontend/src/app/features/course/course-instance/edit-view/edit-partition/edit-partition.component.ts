...
import {
  EditAssignment,
  EditPartition,
} from '../../../models/edit-partition.model';
...
export class EditPartitionComponent {
  ...
  getGroupByTutorAndAssignment(username: string, assignmentId: string) {
    const partition = this.partition().find(
      (p) => p.tutor.username === username,
    );
    return partition!.assignments.find((a) => a.id === assignmentId)!.groups;
  }

  setGroupByTutorAndAssignment(
    username: string,
    assignmentId: string,
    groupValue: number[],
  ) {
    ...
    partition!.assignments.find((a) => a.id === assignmentId)!.groups =
      groupValue;
  }
}
